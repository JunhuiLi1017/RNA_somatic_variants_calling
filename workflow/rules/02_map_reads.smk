rule map_raw:
	...
	log:
		"{outpath}/02_map/logs/{sample}.{library}.{flowlane}.raw.log"
	params:
		ref=config['reference_dir'],
        gtf_file=config['gtf'],
		reads_len=config['reads_len'],
		prefix="{outpath}/02_map/01_raw/{sample}.{library}.{flowlane}.",
        reads_group=get_reads_group,
	...
	shell:
		...
		--sjdbOverhang {params.reads_len} \
		--outSAMattrRGline {params.reads_group} \
		--sjdbGTFfile {params.gtf_file}  > {log} 2>&1
		"""
...
rule map_raw_index:
	...
	log:
		"{outpath}/02_map/logs/{sample}.{library}.{flowlane}.raw.index.log"
	threads:
...
rule apply_bqsr_stat:
	...
	output:
		...
		idxstat="{outpath}/02_map/06_stats/{sample}.bqsr.idxstat.txt",
...
rule multiqc_mapping:
	input:
		...
		samtools_idxstats=lambda wildcards: [
			f"{wildcards.outpath}/02_map/06_stats/{sample}.bqsr.idxstat.txt"
